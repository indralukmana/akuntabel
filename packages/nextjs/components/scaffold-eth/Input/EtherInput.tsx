 ...
 */
export const EtherInput = forwardRef<HTMLInputElement, CommonInputProps & { usdMode?: boolean }>(
  ({ value, name, placeholder, onChange, disabled, usdMode, label, error, errorMessage }, ref) => {
    ...
    const handleChangeNumber = (newValue: unknown) => {
      if (newValue && typeof newValue === "string" && !SIGNED_NUMBER_REGEX.test(newValue)) {
        return;
      }
      ...
      if (displayUsdMode && typeof newValue === "string") {
      ...
      if (typeof newValue === "string" && (newValue.endsWith(".") || newValue.endsWith(".0"))) {
        setTransitoryDisplayValue(newValue);
      } else {
        setTransitoryDisplayValue(undefined);
      }

      const newEthValue = displayValueToEtherValue(displayUsdMode, newValue as string, nativeCurrencyPrice || 0);
